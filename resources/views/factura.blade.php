<!DOCTYPE HTML>

<html lang="es">

<head>
  <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>La Cuponera SV</title>
    <link href="css/estilo.css" rel="stylesheet" type="text/css" />


</head>

<body>


<div id="opciones">
   <a href="/" class="bar">Inicio</a>
  <a href="login" class="bar">Log in</a>
  <a href="nosotros" class="bar">Nosotros</a>
</div>

            <div id="cuponera" class="cuponera">
                <p1 class="text">La Cuponera SV</p1>
            </div>

 
             <div id="fondo" class="fondo">
                  
                    </div>
              
                    

   <div id="metodo" class="metodo">
                <p1 class="text">tu compra se ha realizado con exito </p1>
                <br>
                 
                  <p1 class="text"> Gracias por confiar en nosotros</p1>
                     <br> <br>
                  <p1 class="text"> :)   El equipo de la cuponera SV</p1>
                   <br>  <br>
                    <p1 class="text"> Puedes generar tu factura desde aca</p1>
                  <br>
            </div>
                   
       

              <form method="get" action="/">  

                    <div id="inicio"> 
                      <input  type="submit" value="Inicio" class="seleccionboton">

                </div>
       
                  </form>

                     <form method="get" action="/">  

                    <div id="factura"> 
                      <input  type="submit" value="generar factura" class="seleccionboton">

                </div>
       
                  </form>




</body>

</html>
